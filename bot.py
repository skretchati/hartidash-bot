...
# Google Drive настройки (нужно будет добавить в переменные Railway)
GOOGLE_DRIVE_CREDENTIALS = os.environ.get("GOOGLE_DRIVE_CREDENTIALS")
if GOOGLE_DRIVE_CREDENTIALS:
    try:
        DRIVE_CREDENTIALS = json.loads(GOOGLE_DRIVE_CREDENTIALS)
    except:
...
class UserData:
    ...
    def add_download(self, user_id, size_bytes=0, via_drive=False):
        user_id = str(user_id)
        if user_id not in self.users:
            self.users[user_id] = {'pref': 'video', 'downloads': 1, 'qr': 0, 'drive_uploads': 0}
        else:
...
async def upload_to_drive(file_path, filename=None):
    """
    Загружает файл в Google Drive и возвращает ссылку для скачивания
    """
    try:
        service = get_drive_service()
        if not service:
            logger.error("Drive сервис не доступен - проверьте credentials")
            return None
        
        # Подготовка метаданных файла
        file_metadata = {
            'name': filename or os.path.basename(file_path),
            'parents': ['root']  # Можно изменить на конкретную папку
        }
        ...
        # Возвращаем прямую ссылку на скачивание
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    drive_status = "✅ Drive подключен" if DRIVE_CREDENTIALS else "⚠️ Drive не настроен (будут только файлы до 50 МБ)"
    ...
    welcome_text = (
        ...
        f"• Файлы больше 50 МБ → загружаю в Google Drive и даю ссылку\n\n"
        f"📊 *Статус:* {drive_status}\n\n"
        f"🚀 *Погнали!*"
    )
                    ...
                    if file_size <= MAX_TELEGRAM_SIZE:
                        ...
                        if DRIVE_CREDENTIALS:
                            ...
                            if drive_link:
                                ...
                                logger.info(f"✅ Ссылка на Drive отправлена")
                                sent_count += 1
                                drive_used = True
                            else:
                                await update.message.reply_text(
...
def main():
    """Запуск бота"""
    print("⚡ Запуск HartiDash с Google Drive интеграцией...")
